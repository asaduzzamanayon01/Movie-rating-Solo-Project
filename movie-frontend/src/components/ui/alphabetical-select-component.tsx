...
const AlphabeticalSelect = ({ options, value = 0, onChange, placeholder }) => {
  ...
  return (
    <div className="relative w-full z-50" ref={dropdownRef}>
      <div
        className="flex items-center justify-between w-full p-2 text-left bg-white border border-gray-300 rounded-md shadow-sm cursor-pointer hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                  ...
                  {options.map((option) => (
                    ...
                    >
                      <input
                        ...
                        onChange={() => handleGenreToggle(Number(option.value))}
                      />
      ...
      {value.length > 0 && (
        <div className="flex flex-wrap mt-2">
          {value.map((v) => {
            const option = options.find((o) => Number(o.value) === v);
            return (
              <span
                key={v}
