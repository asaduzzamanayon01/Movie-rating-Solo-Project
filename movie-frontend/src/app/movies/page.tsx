...
// // Define cache outside of the component to persist across renders
// const cache: { [key: string]: Movie[] } = {};
...
//     query?: string
// ) => {
//     return `page=${pageNum}&genre=${genre ?? ""}&user=${user ?? ""}&query=${
//         query ?? ""
//     }`;
// };
...
//         query?: string
//     ) => {
//         const key = cacheKey(pageNum, genre, user, query);
//         return cache[key];
//     };

//     // Set data in the cache
//     const setCachedMovies = (
//         pageNum: number,
//         movies: Movie[],
//         genre?: string,
//         user?: number,
...
//             // Check for cached data before making an API call
...
//                 query ?? undefined
//             );
//             if (cachedMovies && !reset) {
//                 setMovies((prevMovies) => [...prevMovies, ...cachedMovies]);
//                 setHasMore(cachedMovies.length === 18); // Assuming 18 is the page limit
//                 return;
//             }
...
//                     if (reset) return fetchedMovies;
//                     const newMovies = fetchedMovies.filter(
//                         (newMovie: Movie) =>
//                             !prevMovies.some((m) => m.id === newMovie.id)
//                     );
//                     return [...prevMovies, ...newMovies];
//                 });

//                 setHasMore(fetchedMovies.length === 18); // Assuming 18 is the page limit
//             } catch (error) {
...
//     // Fetch movies when the component first mounts
//     useEffect(() => {
//         const genre = searchParams.get("genre");
//         const query = searchParams.get("query");
//         const user = searchParams.get("user");
//         const userId = user ? parseInt(user, 10) : undefined;
//         setPage(1);
//         loadMovies(1, true); // Load fresh data and reset the list
//     }, [searchParams, pathname, loadMovies]);

//     // Fetch more movies when the page state changes
//     useEffect(() => {
//         if (page > 1) {
//             loadMovies(page);
//         }
...
//                 </h1>
//                 <motion.div
//                     className="movies-list grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6"
//                     initial={{ opacity: 0 }}
//                     animate={{ opacity: 1 }}
...
//                             delete the movie from our servers.
//                         </AlertDialogDescription>
//                     </AlertDialogHeader>
//                     <AlertDialogFooter>
